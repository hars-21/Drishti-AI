...
export function Sidebar({ isOpen, toggle }: SidebarProps) {
	return (
		<>
			...
			<div
				className={cn(
					"fixed inset-0 z-20 bg-slate-900/20 lg:hidden transition-opacity backdrop-blur-sm",
			...
			<aside
				className={cn(
					"fixed lg:relative z-30 flex flex-col bg-white text-slate-600 border-r border-slate-200 transition-all duration-300 ease-in-out font-mono shadow-sm",
			...
			>
				<div className="h-14 flex items-center justify-between px-4 border-b border-slate-200 bg-white">
					<div className="flex items-center space-x-3 overflow-hidden">
						<div className="w-6 h-6 bg-gradient-to-br from-blue-600 to-purple-600 rounded-lg flex items-center justify-center shrink-0 shadow-lg shadow-blue-500/20">
							<div className="w-2 h-2 border-2 border-white rounded-full"></div>
						</div>
						{isOpen && (
							<div className="flex flex-col">
								<span className="font-bold text-slate-900 text-sm tracking-wider">DRISHTI-AI</span>
								<span className="text-[10px] text-blue-600 uppercase tracking-widest">
									Sentinels
								</span>
							</div>
						)}
					</div>
					<button onClick={toggle} className="lg:hidden text-slate-400 hover:text-slate-900">
						<X className="w-5 h-5" />
					</button>
				</div>
						...
						>
							...
							{isOpen && (
								<div className="ml-3 flex-1 flex items-baseline justify-between overflow-hidden">
									...
									<span className="text-[9px] text-slate-500 font-normal ml-2">{item.code}</span>
								</div>
							)}

							{!isOpen && (
								<div className="absolute left-full ml-4 px-2 py-1 bg-white border border-slate-200 text-slate-900 text-xs rounded opacity-0 group-hover:opacity-100 pointer-events-none whitespace-nowrap z-50 shadow-lg">
				...
				<div className="p-2 border-t border-slate-200 bg-white">
					<div
						className={cn(
							"flex items-center p-2 rounded-md bg-emerald-50 border border-emerald-200",
							!isOpen && "justify-center p-1"
						)}
					>
						<div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse shadow-[0_0_8px_#10b981]"></div>
						{isOpen && (
							<span className="ml-3 text-[10px] uppercase font-bold text-emerald-700 tracking-wider">
								System Online
							</span>
						)}
					</div>
					...
					<button
						className={cn(
							"flex items-center w-full mt-2 px-3 py-2 text-slate-500 hover:text-red-600 hover:bg-red-50 rounded-md transition-colors",
